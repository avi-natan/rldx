...
from p_diagnosers import diagnosers, SIF, SN, W, SIFU, SIFU2, SIFU3, SIFU4, SIFU5
...
def write_records_to_excel(records, experimental_filename):
    ...
    for i in range(len(records)):
        ...
        row = [
            ...
            get_ordinal_rank(list(record_i['output']['diagnoses']), list(record_i['output']['ranks']), record_i['execution_fault_mode_name']) if record_i['diagnoser'] in {"SN", "SIF", "SIFU", "SIFU2", "SIFU3", "SIFU4", "SIFU5"} else "Irrelevant",  # 21_O_correct_diagnosis_rank
            ...
            record_i['output']['G_max_size'] if record_i['diagnoser'] in {"SIF", "SIFU", "SIFU2", "SIFU3", "SIFU4", "SIFU5"} else "Irrelevant",  # 32_O_G_max_size
        ]
        rows.append(row)
...
def run_experimental_setup_new(arguments, render_mode, debug_print):
    ...
    total_instances_number = W_instance_number + SN_instance_number + SIF_instance_number + 0 + 0 + SIFU3_instance_number + SIFU4_instance_number + SIFU5_instance_number
    current_instance_number = 1
                    ...
                    for num_candidate_fault_modes_i, num_candidate_fault_modes in enumerate(param_dict['num_candidate_fault_modes']):
                        ...
                        for diagnoser_name_i, diagnoser_name in enumerate(param_dict['diagnoser_names']):
                            ...
                            if diagnoser_name in ["SIF", "SIFU", "SIFU2"] and percent_visible_states < 30:
                                print(f'SKIP\n')
                                continue
                            if diagnoser_name in ["SIFU3", "SIFU4", "SIFU5"] and percent_visible_states < 30:
                                print(f'SKIP\n')
                                continue
